...
# Still anchored to a whole line so an inline "the contents of this chapter"
# never matches, but the header may carry markup: an ATX/AsciiDoc marker
# ("## Contents", "== Contents"), emphasis wrappers ("**Contents**"), and a
# trailing colon. A bare "^\s*X\s*$" missed every Markdown document, because
# Markdown writes its ToC heading as "## Table of Contents" -- the same blind
# spot #91/#92 fixed for chapter headings, which left this sibling pattern
# behind. Adopted from upstream PR #126 (virgiliojr94/book-to-skill) and
# extended to the fullwidth colon and ideographic space, which the CJK
# alternative above needs.
_TOC_PATTERN = re.compile(
    ...
    # The trailing class has to include \r: on Windows the extracted text keeps
    # CRLF, and `$` in MULTILINE matches before the \n, not before the \r. The
    # old `\s*$` covered that by accident; upstream PR #126's `[ \t]*$` does not,
    # so porting it verbatim would have silently stopped detecting every ToC on
    # this fork's primary platform.
    r"[ \t　\r]*[:：]?[ \t　\r]*$",                    # optional colon
...
def claim_workdir(workdir: Path = None) -> Path:
    ...
    Every run defaults to the same directory, so two concurrent extractions
    overwrite each other's ``full_text.txt`` in silence: the second writer wins,
    and the first run then reports its own ``metadata.json`` against text it did
    not produce. That is worse than failing, because nothing in the output says
    the text is wrong. Refuse to start instead, and name the way out.

    Stale locks (holder process gone) are reclaimed automatically, so a crashed
    run does not wedge the next one.

    Adopted from upstream PR #126 (virgiliojr94/book-to-skill). This fork already
    made the default workdir per-user on Windows, which removes the multi-user
    collision but not this one: the same user running two extractions still
    shares one directory.
    """
    workdir = OUTPUT_DIR if workdir is None else workdir
    prepare_output_dir(workdir)
    ...
    if holder is not None:
        raise ExtractionError(
            ...
            "Concurrent runs would overwrite each other's full_text.txt. "
            "Give this run its own directory: set BOOK_SKILL_WORKDIR to a "
            "private path, or wait for the other run to finish."
        )
...
def main():
    ...
    metadata = {
        ...
        # The cleanup step deletes the directory this run actually used. Deriving
        # it again from the environment can name a *different* directory -- a
        # concurrent run's, or the old one if BOOK_SKILL_WORKDIR changed since --
        # and rmtree does not ask twice. Record it here so cleanup reads it back.
    ...
    # The lock only has to outlive the writes above. Releasing here rather than
    # in a finally block is deliberate: a crashed run leaves a stale lock, and
    # `claim_workdir` reclaims those by probing the holder PID.
    release_workdir(OUTPUT_DIR)
