...
Delete **the directory this run actually used**, not one derived again from the
environment: `metadata.json` records it as `workdir`. Re-deriving can name a different
directory — a concurrent run's, or the previous one if `BOOK_SKILL_WORKDIR` changed
since the extraction — and `rm -rf` does not ask twice.

`$META_PATH` below is the `Meta ->` path the extractor printed.
...
try:
    workdir = json.loads(meta.read_text(encoding="utf-8")).get("workdir")
...
if workdir:
    ...
    # No metadata to read back: leave the directory alone and say so. A guess here
    # would be a guess about which directory to delete.
