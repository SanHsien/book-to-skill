...
Running `/book-to-skill your-book.pdf` (or a folder, glob, or list of files) creates a full skill in the user-level cross-agent skills directory `~/.agents/skills/<slug>/` — one copy that Copilot CLI, Amp and Codex discover natively. Under Claude Code the converter also creates a link at `~/.claude/skills/<slug>/` (a directory junction on Windows, a symlink elsewhere) and verifies it by reading it back; a missing link is not a failed run, and the report says what is actually on disk. Host-private and project-local destinations stay available on request:
...
```bash
# Upstream one-command install, any host:
npx skills add virgiliojr94/book-to-skill

# Or clone upstream into your skills folder:
